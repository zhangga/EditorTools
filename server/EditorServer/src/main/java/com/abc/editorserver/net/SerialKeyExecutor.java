 ...
 * 但对于被映射到同一个队列的不同 key 也不会并发了。采用映射方式以后，就可以把队列的总数量固定下来，
 * 不再需要回收队列，只要总的队列数量有足够的并发能力，对于应用来说影响不大。<code>假设队列使用
...
public final class SerialKeyExecutor<K> implements Executor
{
	...
	/**
	 * 取消并且删除任务。
	 * @param task 参数必须是通过 submit 提交任务时返回的结果。
	 */
	public boolean remove(Serial.Task<?> task)
	{
		return task.remove();
	}
	 ...
	 * 队列
	 *
	 * @author U-Demon Created on 2016年10月11日 下午5:04:51
	 * @version 1.0.0
	 */
	public final class Serial
	{
		...
		public Serial()
		{
			header = new Task<>(null, dummyHeader);
			reset();
		}
		...
		public <T> Future<T> submit(K key, Callable<T> callable)
		{
			lock.lock();
			try {
				Task<T> lastTask = addLast(new Task<>(key, callable));
				if (size == 1)
				{
					try {
						SerialKeyExecutor.this.execute(lastTask);
					} catch (RejectedExecutionException e) {
						remove(lastTask);	//roll back
						throw e;
		...
		public void scheduleNext(Task<?> done)
		{
		...
		public int cancel(final K key)
		{
			...
			try {
				...
				Task<?> task = header.next.next;	//skip over scheduled-task.
				while (task != header)
				{
		...
		private <T> Task<T> addLast(Task<T> task)
		{
			task.next = header;
		 ...
		 * 任务
		 *
		 * @author U-Demon Created on 2016年10月11日 下午5:05:11
		 * @version 1.0.0
		 */
		public final class Task<T> extends FutureTask<T>
		{
			...
			public Task(K key, Callable<T> callable)
			{
				super(callable);
				this.key = key;
			}
